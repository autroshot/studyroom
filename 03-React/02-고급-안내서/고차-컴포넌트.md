...
컴포넌트는 프랍을 UI로 변환하는 반면, 고차 컴포넌트는 컴포넌트를 다른 컴포넌트로 변환합니다.
...
`DataSource`를 구독하는 `CommentList`와 `BlogPost` 같은 컴포넌트를 생성하는 함수를 만들 수 있습니다. 이 함수는 구독 데이터를 프랍으로 받는 자식 컴포넌트를 인수로 받습니다. `withSubscription` 함수를 호출해 보겠습니다.
...
첫 번째 매개변수는 래핑될 컴포넌트입니다. 두 번째 매개변수는 주어진 `DataSource`와 현재 프랍에서 원하는 데이터를 검색합니다.

`CommentListWithSubscription`과 `BlogPostWithSubscription`이 렌더링되면 `DataSource`에서 검색된 최신 데이터가 포함된 `data` 프랍이 `CommentList`와 `BlogPost`에 전달됩니다.
...
래핑된 컴포넌트는 렌더링에 사용하는 새 `data` 프랍과 함께 컨테이너의 모든 프랍을 받습니다. HOC는 데이터가 사용되는 방법이나 이유에 관심이 없으며 래핑된 컴포넌트는 데이터의 출처에 관심이 없습니다.

`withSubscription`는 일반적인 함수이므로 원하는 만큼 인수를 추가할 수 있습니다. 예를 들어 래핑된 컴포넌트에서 HOC를 추가로 분리하기 위해 `data` 프랍의 이름을 설정 가능하게 만들 수 있습니다. 또는 `shouldComponentUpdate`를 설정하는 인수나 데이터 소스를 설정하는 인수를 받을 수 있습니다. HOC가 컴포넌트가 정의되는 방식을 완전히 제어하기 때문에 이 모든 것이 가능합니다.

컴포넌트와 마찬가지로 `withSubscription`와 래핑된 컴포넌트 간의 계약은 전적으로 프랍에 기반합니다. 이렇게 하면 래핑된 컴포넌트에 동일한 프랍을 제공하는 한 HOC를 쉽게 교체할 수 있습니다. 예를 들어 데이터 가져오기 라이브러리를 변경할 때 유용할 수 있습니다.
...
// EnhancedComponent는 프랍을 받을 때마다 로그를 출력할 것입니다.
...
HOC와 컨테이너 컴포넌트라는 패턴 사이의 유사점을 발견했을 수 있습니다. 컨테이너 컴포넌트는 상위 수준 문제와 하위 수준 문제 간에 책임을 분리하는 전략의 일부입니다. 컨테이너는 구독이나 상태 같은 것들을 관리하고, UI 렌더링과 같은 것들을 처리하는 컴포넌트에 프랍을 전달합니다. HOC는 구현의 일부로 컨테이너를 사용합니다. HOC는 컨테이너 컴포넌트 정의가 매개변수화된 것으로 생각할 수 있습니다.

## 컨벤션: 래핑된 컴포넌트에 관련 없는 프랍 전달하기

HOC는 컴포넌트에 기능을 추가합니다. HOC가 컴포넌트의 기존 계약을 크게 바꿔서는 안 됩니다. HOC에서 반환된 컴포넌트에는 래핑된 컴포넌트와 유사한 인터페이스가 있어야 합니다.

HOC는 특정 관심사와 관련이 없는 프랍을 통과시켜야 합니다. 대부분의 HOC에는 다음과 같은 렌더링 메서드가 포함되어 있습니다.

```jsx
render() {
  // 이 HOC에만 해당되며 통과해서는 안 되는 추가 프랍을 필터링합니다.
  const { extraProp, ...passThroughProps } = this.props;

  // 래핑된 컴포넌트에 프랍을 주입합니다. 일반적으로 상태 값 또는 인스턴스 메서드입니다.
