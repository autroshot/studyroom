...
HOC는 컴포넌트에 기능을 추가한다. HOC가 컴포넌트의 기존 계약을 크게 바꿔서는 안 된다. HOC에서 반환된 컴포넌트에는 래핑된 컴포넌트와 유사한 인터페이스가 있어야 한다.

HOC는 특정 관심사와 관련이 없는 프랍을 통과시켜야 한다. 대부분의 HOC에는 다음과 같은 렌더링 메서드가 포함되어 있다.

```jsx
render() {
  // 이 HOC에만 해당되며 통과해서는 안 되는 추가 프랍을 필터링한다.
  const { extraProp, ...passThroughProps } = this.props;

  // 래핑된 컴포넌트에 프랍을 주입한다. 일반적으로 상태 값 또는 인스턴스 메서드이다.
