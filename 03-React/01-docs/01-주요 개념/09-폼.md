...
폼 요소에 `value`  속성이 설정되었기 때문에, 표시되는 값은 항상  `this.state.value`가 되고 React 상태는 신뢰 가능한 단일 출처(single source of truth)가 된다. 키 입력이 있을 때마다 React 상태를 업데이트하는 `handleChange`가 동작하기 때문에, 사용자가 입력할 때마다 표시되는 값이 업데이트된다.
