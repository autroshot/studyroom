...
1. 자동 생성된 `id`(예: `20`)로 `User`를 생성한다.
...
관계 필드는 스칼라 타입이 없는 프리즈마 모델의 필드이다. 대신 관계 필드의 타입은 다른 모델이다.
...
> **참고**
>
> [관계를 명확하게](#명확한-관계) 해야 하는 경우가 아니면 암시적 다대다 관계에는 `@relation` 속성이 필요하지 않다.
...
이 기능을 사용하려면 대신 명시적 다대다를 설정해야 한다.

암시적 m-n 관계는 여전히 밑에 있는 데이터베이스의 관계 테이블에 나타난다. 그러나 이 관계 테이블은 프리즈마에서 관리한다.

명시적 m-n 관계 대신 암시적 관계를 사용하면 다대다 관계에 대한 프리즈마 클라이언트 API가 좀 더 간단해진다. 예를 들어 [중첩 쓰기](https://www.prisma.io/docs/concepts/components/prisma-client/relation-queries#nested-writes) 내부에 중첩 수준이 하나 줄어든다.
...
`posts`와 `author` 둘 다 타입이 스칼라가 아닌 다른 모델이기 때문에 관계 필드이다.
