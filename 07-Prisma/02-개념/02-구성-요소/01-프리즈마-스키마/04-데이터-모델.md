...
모델 간의 관계에 대한 자세한 예시와 정보는 [관계 문서](https://www.prisma.io/docs/concepts/components/prisma-schema/relations)를 참조합니다.
...
타입스크립트를 사용할 때 이러한 타입 정의는 모든 데이터베이스 쿼리가 완전히 타입에 안전하고 컴파일 타임에 유효성이 검사되도록 합니다. 이는 `select`나 `include`를 사용하는 부분적인 쿼리에도 적용됩니다.
...
관계 필드 `posts`와 `profile`은 기본적으로 타입 정의에 포함되지 않습니다. 그러나 `User` 타입의 변형이 필요한 경우에는 프리즈마 클라이언트에서 생성한 일부 헬퍼 타입을 사용하여 정의할 수 있습니다. 이 경우 이러한 헬퍼 타입은 `UserGetIncludePayload`와 `UserGetSelectPayload`라고 부른다.
