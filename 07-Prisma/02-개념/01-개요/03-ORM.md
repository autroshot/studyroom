...
프로젝트를 처음부터 시작한다고 가정하면 전체 워크플로는 다음과 같습니다. 데이터베이스 스키마에 `User` 테이블을 만들고 위의 예시와 같이 `User` 개체 클래스를 정의하는 마이그레이션을 만듭니다.

프로젝트가 진행되어 테이블에 새 `salutation` 열을 추가하기로 결정했다면 `User` 테이블을 변경하고 `salutation` 열을 추가하는 또 다른 마이그레이션을 생성합니다.
...
마이그레이션이 수행되고 데이터베이스 스키마가 변경된 후에는 개체 및 매퍼 클래스도 새 `salutation` 열을 알도록 업데이트해야 합니다.

타입ORM에서는 이것은 `User` 개체 클래스에 `salutation` 프로퍼티를 추가하는 것을 의미합니다.
...
요약하면 스키마를 발전시키는 것은 앱 구축의 핵심 부분입니다. ORM의 경우, 스키마 업데이트 워크플로에는 마이그레이션 도구를 사용하여 마이그레이션을 만든 다음 해당 개체와 매퍼 클래스를 업데이트하는 작업이 포함됩니다(구현에 따라 다름). 나중에 배우겠지만 프리즈마는 이에 대해 다른 접근 방식을 취합니다.
...
프리즈마 클라이언트에서 관계 가져오기는 `include` 옵션으로 사용합니다.
