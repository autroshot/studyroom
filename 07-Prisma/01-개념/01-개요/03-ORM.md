...
프리즈마는 ORM(Object Relational Mapping)이다. 하지만 Prisma는 전통적인 ORM과 근본적으로 다르며 기존의 ORM에서 많이 발생하는 문제를 겪지 않을 수 있는 새로운 종류의 ORM이다.
...
Prisma는 기존 ORM과 근본적으로 다르다. Prisma를 사용하면 데이터베이스 스키마와 프로그래밍 언어의 모델에 대한 단일 정보 소스 역할을 하는 선언적 [Prisma 스키마](https://www.prisma.io/docs/concepts/components/prisma-schema)에서 모델을 정의할 수 있다. 그런 다음 앱의 코드에서 Prisma Client를 사용하여 복잡한 모델 인스턴스를 관리하는 오버헤드 없이 안전한 타입 방식으로 데이터베이스의 데이터를 읽고 쓸 수 있다. Prisma Client가 항상 순수한 자바스크립트 객체를 반환하기 때문에 데이터 쿼리 프로세스를 훨씬 더 자연스럽고 예측 가능하게 만든다.

이제 ORM 패턴과 워크플로, Prisma가 데이터 매퍼 패턴을 구현하는 방법, Prisma 접근 방식의 이점에 대해 자세히 알아볼 것이다.

## ORM이 무엇일까?

### ORM 패턴 - 액티브 레코드와 데이터 매퍼

ORM은 높은 수준의 데이터베이스 추상화를 제공한다. ORM은 데이터베이스의 복잡성을 숨기면서 데이터를 생성, 읽기, 삭제, 조작하기 위해 객체를 통해 프로그래밍 방식의 인터페이스를 제공한다.

ORM의 아이디어는 모델을 데이터베이스의 테이블에 매핑하는 **클래스**로 정의한다는 것이다. 클래스와 인스턴스는 데이터베이스에서 데이터를 읽고 쓸 수 있는 프로그래밍 방식의 API를 제공한다.

두 가지 일반적인 ORM 패턴이 있다. [액티브 레코드](https://en.wikipedia.org/wiki/Active_record_pattern)와 [데이터 매퍼](https://en.wikipedia.org/wiki/Data_mapper_pattern)는 객체와 데이터베이스 간에 데이터를 전송하는 방식이 다르다. 두 패턴 모두 클래스를 기본 구성 요소로 정의해야 한다. 두 패턴의 가장 큰 차이점은 데이터 매퍼 패턴이 데이터베이스에서 앱 코드의 메모리 내 객체를 분리하고 데이터 매퍼 계층을 사용하여 둘 사이에서 데이터를 전송한다는 것이다. 실제로 데이터 매퍼를 사용하면 메모리 내 객체(데이터베이스의 데이터)는 데이터베이스가 있는지조차 모른다.
...
데이터 매퍼 ORM을 사용하면 코드에 구현된 문제 영역과 데이터베이스 간의 유연성을 높일 수 있다. 데이터 매퍼 패턴을 사용하면 전체 데이터 매핑 레이어 뒤에 있는 도메인에 대해 생각하는 이상적인 방법이 아닌 데이터베이스가 구현되는 방식을 숨길 수 있기 때문이다.
...
그런 다음 프로젝트가 진행되고 테이블에 새 `salutation` 열을 추가하기로 결정하면 `User` 테이블을 변경하고 `salutation` 열을 추가하는 또 다른 마이그레이션을 생성한다.
...
요약하면 스키마를 발전시키는 것은 앱 구축의 핵심 부분이다. ORM의 경우, 스키마 업데이트 워크플로에는 마이그레이션 도구를 사용하여 마이그레이션을 만든 다음 해당 엔터티와 매퍼 클래스를 업데이트하는 작업이 포함된다(구현에 따라 다름). 나중에 배우겠지만 Prisma는 이에 대해 다른 접근 방식을 취한다.
...
이제 ORM의 비용과 이점을 배웠으므로 Prisma가 무엇인지 더 잘 이해할 수 있을 것이다.

## 프리즈마

Prisma는 앱 개발자의 데이터베이스 작업을 돕고 다음의 도구를 제공하는 차세대 ORM이다.

- **Prisma Client** - 앱에서 사용할 수 있게 자동 생성되며 타입이 안전한 데이터베이스 클라이언트
- **Prisma Migrate** - 선언적 데이터 모델링 및 마이그레이션 도구
- **Prisma Studio** - 데이터베이스의 데이터를 검색하고 관리하기 위한 최신 GUI

> **참고**
>
> Prisma Client는 가장 눈에 띄는 도구이기 때문에 간단히 Prisma라고 부르기도 한다.

세 개의 도구는 데이터베이스 스키마, 앱의 객체 스키마, 둘 사이의 매핑에 대한 단일 정보 소스로 [Prisma 스키마](https://www.prisma.io/docs/concepts/components/prisma-schema)를 사용한다. 프리즈마 스키마는 개발자에 의해 정의되며 Prisma의 주요 설정 파일이다.

Prisma는 타입 안전성, 풍부한 자동 완성, 관계 가져오기를 위한 기본 API와 같은 기능을 사용하여 개발 중인 소프트웨어의 생산성과 신뢰성을 향상시킨다.

### Prisma가 데이터 매퍼 패턴을 구현하는 방법

앞에서 언급했듯이 Data Mapper 패턴은 데이터베이스와 앱을 서로 다른 팀에서 소유하고 있는 조직과 잘 어울린다.

관리형 데이터베이스 서비스 및 DevOps 방식이 포함된 최신 클라우드 환경의 등장으로 더 많은 팀이 교차 기능 접근 방식을 채택하고 있다.

Prisma를 사용하면 DB 스키마와 객체 스키마를 동시에 발전시킬 수 있으므로 처음부터 편차의 필요성을 줄이면서 `@map` 속성을 사용하여 앱과 데이터베이스를 어느 정도 분리된 상태로 유지할 수 있다. 이것이 한계처럼 보일 수 있지만, 이는 도메인 모델의 진화(객체 스키마를 통한)가 나중에 데이터베이스에 부과되는 것을 방지한다.

Prisma의 Data Mapper 패턴 구현이 기존 Data Mapper ORM과 개념적으로 어떻게 다른지 이해하기 위해 개념과 구성 요소를 간략하게 비교해 보겠다.

| 개념                | 설명                                            | 기존 ORM의 구성 요소                                      | Prisma의 구성 요소                    | Prisma의 믿을 만한 소스   |
| :------------------ | :---------------------------------------------- | :-------------------------------------------------------- | :------------------------------------ | :------------------------ |
| 객체 스키마         | 앱의 인메모리 데이터 구조                       | 모델 클래스                                               | 생성된 타입스크립트 타입              | Prisma 스키마의 모델      |
| 데이터 매퍼         | 객체 스키마와 데이터베이스 사이를 변환하는 코드 | 매퍼 클래스                                               | Prisma Client에서 생성된 함수         | Prisma 스키마의 @map 속성 |
| 데이터베이스 스키마 | 데이터베이스의 데이터 구조(예: 테이블과 열)     | 직접 작성하거나 프로그래밍 방식 API를 사용하여 작성한 SQL | Prisma 마이그레이트에 의해 생성된 SQL | 프리즈마 스키마           |

Prisma는 다음과 같은 추가 이점과 함께 Data Mapper 패턴을 구현한다.

- Prisma 스키마를 기반으로 Prisma 클라이언트를 생성하여 클래스 정의 및 매핑 로직의 보일러플레이트를 줄임
