...
프리즈마는 ORM(Object Relational Mapping)이다. 하지만 프리즈마는 전통적인 ORM과 근본적으로 다르며 기존의 ORM에서 많이 발생하는 문제를 겪지 않을 수 있는 새로운 종류의 ORM이다.
...
- **프리즈마 스튜디오** - 데이터베이스의 데이터를 검색하고 관리하기 위한 최신 GUI
...
프리즈마는 타입 안전성, 풍부한 자동 완성, 관계 가져오기를 위한 기본 API와 같은 기능을 사용하여 개발 중인 소프트웨어의 생산성과 신뢰성을 향상시킨다.
...
프리즈마를 사용하면 DB 스키마와 객체 스키마를 동시에 발전시킬 수 있으므로 처음부터 편차의 필요성을 줄이면서 `@map` 속성을 사용하여 앱과 데이터베이스를 어느 정도 분리된 상태로 유지할 수 있다. 이것이 한계처럼 보일 수 있지만, 이는 도메인 모델의 진화(객체 스키마를 통한)가 나중에 데이터베이스에 부과되는 것을 방지한다.

프리즈마의 Data Mapper 패턴 구현이 기존 Data Mapper ORM과 개념적으로 어떻게 다른지 이해하기 위해 개념과 구성 요소를 간략하게 비교해 보겠다.
...
프리즈마는 다음과 같은 추가 이점과 함께 Data Mapper 패턴을 구현한다.
...
프리즈마의 Data Mapper 패턴 구현의 중심에는 프리즈마 스키마가 있다. 이는 다음 책임에 대한 단일 정보 소스이다.
...
프리즈마의 모델은 액티브 레코드 ORM과 의미가 조금 다르다. 프리즈마에서 모델은 프리즈마 클라이언트의 속성에 대한 열 간의 매핑, 관계, 테이블을 설명하는 추상 엔티티로 프리즈마 스키마에 정의된다.
...
- 필드에는 다음을 정의하는 필드 속성이 포함될 수 있다.
  ...
  - `@unique` 속성이 있는 고유 키
  - `@default` 속성이 있는 기본값
  - `@map` 속성이 있는 테이블 열과 프리즈마 클라이언트 필드 간의 매핑. 예를 들어 `content` 필드(프리즈마 클라이언트에서 액세스 가능)가 데이터베이스의 `post_content` 열에 매핑된다.
...
3. (선택 사항) 프리즈마 클라이언트와 데이터베이스 간의 [필드와 모델 매핑](https://www.prisma.io/docs/concepts/components/prisma-schema/data-model#mapping-model-names-to-tables-or-collections)을 커스터마이징한다.
4. `prisma generate`를 실행한다.

프리즈마는 `node_modules` 폴더 안에 프리즈마 클라이언트를 생성하여 앱에서 가져오는 것이 가능해진다. 보다 광범위한 사용 설명서는 [프리즈마 클라이언트 API](https://www.prisma.io/docs/concepts/components/prisma-client) 문서를 참조한다.
...
마지막 단계는 프리즈마 스키마에 필드를 추가하고 프리즈마 마이그레이트를 사용하여 데이터베이스 스키마를 원하는 상태로 변환하여 선언적 마이그레이션이 작동하는 방식을 보여준다. 마이그레이션이 실행된 후 프리즈마 클라이언트는 업데이트된 스키마를 반영하도록 자동으로 재생성된다.
...
프리즈마 클라이언트를 사용하여 데이터베이스에 접근하는 것은 노출되는 쿼리 메소드를 통해 가능하다. 모든 쿼리는 오래된 순수 자바스크립트 객체를 반환한다.
...
이 쿼리에서 `findUnique` 메서드는 `User` 테이블에서 하나의 행을 가져오는 데 사용된다. 기본적으로 프리즈마는 `User` 테이블의 모든 스칼라 필드를 반환한다.
...
요약하면 프리즈마는 전통적인 ORM과 다르며 기존 ORM에서 일반적으로 발생하는 문제를 해결한 새로운 종류의 Data Mapper ORM이라는 것이다.
...
모든 추상화와 마찬가지로 프리즈마와 다른 ORM은 서로 다른 가정으로 데이터베이스의 세부 정보를 숨긴다.
