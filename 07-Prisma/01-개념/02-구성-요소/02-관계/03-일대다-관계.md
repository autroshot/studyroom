...
이전 예시에서 `Post` 모델의 `author` 관계 필드는 `User` 모델의 `id` 필드를 참조한다. 다른 필드를 참조할 수도 있다. 이 경우에는 각 `Post`에 하나의 `User`만 연결된다는 것을 보장하기 위해 `@unique` 속성을 필드에 붙여야 한다.
...
1-n 관계의 목록 쪽은 항상 **필수**이다. 관계의 다른 쪽에서 주석이 달린 관계 필드와 관계 스칼라는 둘 다 선택 사항이거나 필수일 수 있다.
