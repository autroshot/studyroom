...
- 열거형 멤버는 상수 열거형 표현식으로 초기화된다. 상수 열거형 표현식은 컴파일 시 알아낼 수 있는 TypeScript 표현식의 일부이다. 다음의 경우 상수 열거형 표현식이라고 한다.
...
아직 [유니언 타입](https://typescript-kr.github.io/pages/advanced-types.html#유니언-타입-union-types)에 대해서 배우진 않았지만, 유니언 열거형을 사용하면 타입 시스템이 열거형 자체에 존재하는 정확한 값의 집합을 알고 있다는 사실을 활용할 수 있다. 이 덕분에 TypeScript는 다음과 같이 값을 잘못 비교하는 버그를 잡을 수 있다.
...
TypeScript는 다음과 같은 JavaScript 코드로 컴파일한다.
