...
<이미지 server-side-rendering-with-data.png>
...
<이미지 client-side-rendering.png>

이 접근 방식은 적절한 예시로는 사용자 대시보드 페이지가 있다. 대시보드는 사용자별 비공개 페이지이므로 SEO와 관련이 없으며 페이지를 미리 렌더링 할 필요가 없다. 데이터는 자주 업데이트되므로 요청 타임에서의 데이터 가져오기가 필요하다.
