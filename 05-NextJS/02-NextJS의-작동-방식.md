...
개발자는 일반적으로 애플리케이션을 다른 URL에서 액세스할 수 있는 여러 페이지로 분할한다. 이러한 각 페이지는 애플리케이션에 대한 고유한 **진입점(entry point)**이 된다.
...
우리는 이 서버를 CDN 서버와 에지 서버와 같이 앱 코드가 배포될 수 있는 다른 장소와 구별하기 위해 오리진(origin)이라는 용어를 사용한다.
