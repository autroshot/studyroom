...
# 02. 코드 구조
## 문
문(statement)은 어떤 작업을 수행하는 <strong>문법 구조(syntax structure)</strong>와 <strong>명령어(command)</strong>를 의미한다. 서로 다른 문은 세미콜론(;)으로 구분된다.

세미콜론을 생략해도 '세미콜론 자동 삽입'이 되지만 세미콜론은 반드시 넣는 것이 규칙이다.

## 주석
주석에는 한 줄 주석과 여러 줄 주석이 있다.
```js
// 한 줄 주석
...
```
주로 프레임워크에서 사용되는 것을 볼 수 있다. 가독성이 떨어지기 때문에 코드 작성 시에는 사용하지 않아야 한다.
<br></br>
# 09. 비교 연산자
...
## 일치 연산자
일치 연산자(===)와 불일치 연산자(!==)를 사용하면 형 변환 없이 값을 비교할 수 있다. 이 연산자를 사용하면 에러가 발생할 확률을 줄일 수 있다.
```js
alert(0 == false); // true
alert(0 === false); // false
```
...
### 함수 선언
다음과 같이 함수를 만드는 방식을 함수 선언(function declaration), 혹은 함수 선언문 방식이라고 부른다.
```js
...
alert( userName ); 
```
- 전역 변수 대신 **지역 변수와 매개 변수를 활용**하는 것이 좋다.
...
```
- 다음과 같이 기본값을 함수 내부에서 설정하는 것도 가능하다.
```js
...
### 반환 값
함수를 호출했을 때 함수는 특정 값을 반환할 수 있으며 그 값을 반환 값(return value)이라고 부른다.
```js
function sum(a, b) {
  return a + b;
}
...
- 이름만 보고도 어떤 동작을 하는지 알 수 있는 코드를 자기 설명적(self-describing) 코드라고 부른다.
<br></br>
# 16. 함수 표현식
자바스크립트는 함수를 특별한 종류의 값으로 취급한다. 따라서 다음과 같이 함수 표현식(Function Expression)을 사용해서 함수를 만들 수 있다.
```js
...
```
- 함수는 값이기 때문에 변수를 복사해 다른 변수에 할당할 수도 있다.
```js
...
```
### 콜백 함수
함수를 함수의 인수로 전달하고, 필요하다면 인수로 전달한 그 함수를 "**나중에 호출(called back)**"하는 것이다. 매개변수로 동작을 전해준다고 할 수 있다.
```js
...
```
- ***문자열이나 숫자 등의 일반적인 값들은 데이터를 나타내고, 함수는 하나의 동작을 나타낸다.***

### 익명 함수
이름 없이 선언한 함수를 익명 함수(anonymous function)라고 부른다. 적절한 곳에 익명 함수를 사용하면 코드의 길이를 줄일 수 있다.
```js
function ask(question, yes, no) {
  if (confirm(question)) yes()
  else no();
...
welcome(); // 제대로 동작한다.
```
- 함수 선언문을 사용하는 것이 부적절할 때에만 함수 표현식을 사용하는 게 좋다. 함수 선언문을 사용하는 것이 코드 구성이 더 자유로워지고 가독성도 좋아지기 때문이다.
<br></br>
# 17. 화살표 함수 기본
화살표 함수(arrow function)를 이용하면 함수 표현식보다 더 단순하고 간결한 문법으로 함수를 만들 수 있다.
```js
