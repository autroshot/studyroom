...
자바스크립트 배열을 사용하면 큐나 스택을 모두 만들 수 있다. 이 자료구조들은 배열의 처음이나 끝에 요소를 더하거나 뺄 때 사용한다. 이렇게 처음이나 끝에 요소를 더하거나 빼주는 연산을 제공하는 자료구조를 컴퓨터 과학 분야에서는  데큐(deque, Double Ended Queue)라고 부른다.

**아래는 배열의 끝에 무언가를 해주는 메서드이다.**
...
## 반복문
`for`문은 배열을 순회할 때 쓰는 가장 오래된 방법이다. 순회할 때 인덱스를 사용한다.
```js
let arr = ['사과', '오렌지', '배'];
