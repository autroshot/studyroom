# 속성과 프로퍼티
브라우저는 웹페이지를 만나면 HTML을 읽어서(parsing) DOM 객체를 생성한다. 요소 노드(element node)에서 대부분의 표준 HTML 속성(attribute)은 DOM 객체의 프로퍼티(property)가 된다. 대부분의 속성-프로퍼티는 일대일로 매핑되지만 일부 예외가 존재한다.
